...
const ChatScreen = ({filteredMessagesList}) => {
  return (
    <div
        id="screen"
        className="w-5/6 border-amber-400 flex flex-col-reverse border-2 bg-yellow-200 rounded-lg mt-6 p-8 overflow-auto shadow-lg"
      >
        <ul>
          {filteredMessagesList.map((messageItem, index) => (
            <li key={index}>
              {messageItem.senderSocketId === "me" ? (
                <div
                  id="right"
                  className="md:text-lg bg-teal-400 max-w-fit p-3 mb-4 rounded-lg ml-auto break-all whitespace-pre-wrap"
                >
